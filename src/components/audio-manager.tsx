...
import { UrlInput } from './modal/url-input'
...
function SettingsTile({ icon, className, transcriber }: SettingsTileProps) {
  ...
  return (
    <div className={className}>
      <Tile icon={icon} onClick={onClick} />
      <SettingsModal
        show={showModal}
        onSubmit={onSubmit}
        onClose={onClose}
...
function SettingsModal({ show, onClose, transcriber }: SettingsModalProps) {
        ...
        <>
          ...
          >
            {Object.keys(models)
              .filter(
                (key) =>
                  transcriber.quantized ||
                  // @ts-ignore
                  models[key].length == 2
              )
              .filter(
                (key) => (
                  !transcriber.multilingual || !key.startsWith('distil-whisper/')
                )
              )
              .map((key) => (
                <option key={key} value={key}>
                  {`${key}${
                    (transcriber.multilingual || key.startsWith('distil-whisper/')) ? '' : '.en'
                  } (${
                    // @ts-ignore
                    models[key][transcriber.quantized ? 0 : 1]
                  }MB)`}
                </option>
              ))}
          </select>
          ...
          <div className='flex justify-between items-center mb-3 px-1'>
            ...
            <div className='flex'>
              <input
                id='quantize'
                type='checkbox'
                checked={transcriber.quantized}
                onChange={(e) => {
                  transcriber.setQuantized(e.target.checked)
                }}
              ></input>
...
function UrlTile(props: UrlTileProps) {
  const [showModal, setShowModal] = useState(false)
...
function UrlModal(props: UrlModalProps) {
...
function RecordTile(props: RecordTileProps) {
  const [showModal, setShowModal] = useState(false)
